# ADR-05: Patrones de diseño GOF (Decorator y Factory Method)
...
El **Decorator** se eligió porque la arquitectura hexagonal ya define el puerto `IClienteRepository`. Eso permite envolver la implementación real con otra clase que cumple la misma interfaz y le agrega logging, **sin tocar ni una línea** del repositorio original. Además, el logging se puede activar o desactivar solo cambiando el registro en la inyección de dependencias, cumpliendo el principio *Open/Closed*: el repositorio queda abierto a extensión pero cerrado a modificación.
...
| **Programación orientada a aspectos (AOP)** | Resuelve el *cross-cutting concern*, pero agrega una librería y complejidad innecesaria para el alcance del proyecto y una desarrolladora principiante. |
...
- **Negocio**: La auditoría de operaciones y los reportes en varios formatos aportan valor directo al dueño para analizar la operación, sin reescribir el núcleo del sistema.

**⚠️ Lo que sacrifico o asumo:**

- **Técnica**: Se agregan más clases e indirección. Para alguien no familiarizado con GOF, seguir el flujo requiere entender primero el patrón. Es complejidad asumida a cambio del desacoplamiento que se gana.
- **Deuda o riesgo**: El Decorator de logging solo se aplica al repositorio de Clientes; auditar también Operadores, Unidades y Servicios requeriría sus propios decoradores. Igualmente, el Factory Method de reportes podrá extenderse a Servicios en una fase posterior. Es una deuda de cobertura asumida para el alcance actual.
...
│       └── ReporteCreator.cs            (creator abstracto + ReporteTextoCreator, ReporteCsvCreator)
│
